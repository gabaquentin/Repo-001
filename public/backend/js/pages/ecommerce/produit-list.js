...
$(document).ready(function () {
    let a = $("#datatable-buttons").DataTable({
        ...
        columns: [
            ...
            { data: 'nom' },
            { data: 'typeTransaction',"orderable": false },
            { data: 'prix' },
            { data: 'prixPromo' },
            { data: 'localisation',"orderable": false },
            { data: 'creation',"orderable": false },
        ...
        columnDefs: [
            ...
            },
            {
                targets: 7,
                render: function(data, type, row) {
                    return `<b class="text-danger">${formatPhpDate(row.date.dateAjout,true)}</b>`;
                }
            },
            {
                targets: -1,
    ...
    a.on('click', '.delete-p', function (e) {
        e.preventDefault();
        ajaxDeleteProduit($(this).data("id"));
    });
